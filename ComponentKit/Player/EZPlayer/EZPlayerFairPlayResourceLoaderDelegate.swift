...
private extension EZPlayerFairPlayResourceLoaderDelegate {
    ...
    func prepareAndSendContentKeyRequest(resourceLoadingRequest: AVAssetResourceLoadingRequest) {
        ...
        do {
          ...
          printLog("Error obtaining key request data: \(error.domain) reason: \(String(describing: error.localizedFailureReason))")
            resourceLoadingRequest.finishLoading(with: error)
            return
        }
        ...
        if shouldPersist, #available(iOS 9.0, *){
          ...
          guard let  persistentContentKeyData = try? resourceLoadingRequest.persistentContentKey(fromKeyVendorResponse: ckcData, options: nil) else {
            ...
            resourceLoadingRequest.finishLoading()
            resourceLoadingRequest.finishLoading(with: nil)
            return
          }
